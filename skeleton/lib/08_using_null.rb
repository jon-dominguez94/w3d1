...
def all_teachers_join
  ...
  execute(<<-SQL)
  SELECT t.name, d.name FROM teachers AS t LEFT OUTER JOIN depts AS d ON t.dept_id = d.id
  SQL
end
...
def all_depts_join
  ...
  execute(<<-SQL)
  SELECT t.name, d.name FROM depts AS d LEFT OUTER JOIN teachers AS t ON t.dept_id = d.id
  SQL
end
...
def teachers_and_depts
  ...
  # department.
  execute(<<-SQL)
  SELECT t.name, COALESCE(d.name, 'None') FROM teachers t LEFT OUTER JOIN depts d ON t.dept_id = d.id
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
  SELECT d.name, COUNT(t.dept_id) FROM depts d LEFT OUTER JOIN teachers t ON d.id = t.dept_id GROUP BY d.name
  SQL
end
...
def teachers_and_divisions
  ...
  execute(<<-SQL)
  SELECT name, CASE WHEN dept_id = 1 THEN 'Sci' WHEN dept_id = 2 THEN 'Sci' ELSE 'Art' END FROM teachers
  SQL
end
...
def teachers_and_divisions_two
  ...
  # 'None' otherwise.
  execute(<<-SQL)
  SELECT name, CASE WHEN dept_id = 1 THEN 'Sci' WHEN dept_id = 2 THEN 'Sci' WHEN dept_id = 3 THEN 'Art' ELSE 'None' END FROM teachers
  SQL
end
