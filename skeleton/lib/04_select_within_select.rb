...
def richer_than_england
  ...
  # 'United Kingdom'.
  execute(<<-SQL)
  SELECT name FROM countries WHERE continent = 'Europe' and gdp/population > (SELECT gdp/population FROM countries WHERE name = 'United Kingdom')
  SQL
end
...
def population_constraint
  ...
  execute(<<-SQL)
  SELECT name,population from countries WHERE population > (select population from countries where name = 'Canada') and population < (select population from countries where name = 'Poland' )
  SQL
end
...
def sparse_continents
  ...
  select name,continent,population from countries where continent IN (SELECT continent From countries group by continent having max(population) < 25000000 )
